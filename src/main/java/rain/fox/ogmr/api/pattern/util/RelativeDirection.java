...
/**
 * 「相对方向」枚举：{@code UP/DOWN/LEFT/RIGHT/FRONT/BACK}。
 *
 * <p>
 * 从 GTM 的 {@code com.gregtechceu.gtceu.api.pattern.util.RelativeDirection} 拆出。
 * 结构图案（{@code FactoryBlockPattern.start(charDir, stringDir, aisleDir)}）就是用它来描述
 * 字符串数组的三个轴各自朝哪，控制器旋转时靠它换算成真实世界方向。
 *
 * <p>
 * 与 GTM 版本的差异（去 GT 化 + JDK17 友好）：
 * <ul>
 * <li>去掉 {@code GTUtil.DIRECTIONS} 依赖，{@link #BY_GLOBAL_DIRECTION} 直接按
 * {@code Direction.values().length} 开数组；</li>
 * <li>去掉 {@code org.jetbrains.annotations.ApiStatus} 的 {@code @ScheduledForRemoval}
 * （仅保留 {@code @Deprecated}），其余逻辑、常量、方法与 GTM 一致。</li>
 * </ul>
 */
public enum RelativeDirection implements StringRepresentable {
    ...
    /**
     * 用 ordinal 计算实际方向，避免中间 Direction 对象创建（BlockPattern 热路径用）。
     */
    public int getActualOrdinal(int facingOrdinal) {
        return actualDirection.apply(Direction.values()[facingOrdinal]).ordinal();
    }
    ...
    /**
     * @deprecated 重命名为 {@link RelativeDirection#getRelative(Direction, Direction, boolean) getRelative}。
     */
