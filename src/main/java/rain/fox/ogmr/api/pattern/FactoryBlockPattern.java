...
/**
 * 结构图案的 DSL 构造器：{@code FactoryBlockPattern.start().aisle(...).where(...).build()}。
 *
 * <p>
 * 从 GTM 的 {@code com.gregtechceu.gtceu.api.pattern.FactoryBlockPattern} 拆出。
 * 逻辑与 GTM 完全一致（含三条轴必须互不相同的校验、aisle 宽高校验、
 * {@code aisleRepeatable}/{@code setRepeatable} 的重复轴支持、{@code where} 对 any/air 的特殊处理、
 * {@code build()} 里根据 controller 字符推算 {@code centerOffset} 的逻辑）。
 *
 * <p>
 * 与 GTM 版本的差异：只把 {@code Predicates} / {@code RelativeDirection} 换成本库包路径，
 * 没有行为改动。
 */
public class FactoryBlockPattern {
    ...
    /** 默认轴向：字符行 → LEFT，字符串（行内）→ UP，aisle → FRONT。 */
    ...
    /** 把 DSL 编译成可用的 {@link BlockPattern}（这一步会检查有没有漏配的字符）。 */
